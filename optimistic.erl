...
handle_call({commit_from_loop, Ref, LoopPid, {_Res, Change}}, _From, State) ->
	Me = self(),
...
loop(Ref, Creator, State) ->
	...
	receive
		{Ref, Creator, {operation, OFun, ViewMap}} ->
			...
			loop(Ref, Creator, ([waiting, Fun_Pid] ++ State));
